...
from sqlova.model.nl2sql.wikisql_models import Seq2SQL_v1, Loss_sw_se
...
def construct_hyper_param(parser):
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="The number of accumulation of backpropagation to effectivly increase the batch size.",  # NOQA: E501
    )
    parser.add_argument(
        "--fine_tune",
        default=False,
    ...
    parser.add_argument(
        "--model_type", default="Seq2SQL_v1", type=str, help="Type of model."
    )
    ...
    parser.add_argument(
        "--max_seq_length",
        default=222,
        type=int,  # Set based on maximum length of input tokens.
    ...
    parser.add_argument(
        "--no_pretraining", action="store_true", help="Use BERT pretrained model"
    )
    parser.add_argument(
        "--bert_type_abb",
        default="uS",
    ...
    parser.add_argument(
        ...
        default=100,
        type=int,
        help="The dimension of hidden vector in the seq-to-SQL module.",
    )
    ...
    parser.add_argument(
        "--beam_size", type=int, default=4, help="The size of beam for smart decoding"
    )
    ...
    if (
        args.bert_type_abb == "cS"
        or args.bert_type_abb == "cL"
        or args.bert_type_abb == "mcS"
    ):
...
def get_models(
    args, BERT_PT_PATH, trained=False, path_model_bert=None, path_model=None
):
    # some constants
    ...
    # Get BERT
    model_bert, tokenizer, bert_config = get_bert(
        BERT_PT_PATH, args.bert_type, args.do_lower_case, args.no_pretraining
    )
    args.iS = (
        bert_config.hidden_size * args.num_target_layers
    )  # Seq-to-SQL input vector dimenstion
...
def get_data(path_wikisql, args):
    ...
    train_loader, dev_loader = get_loader_wikisql(
        train_data, dev_data, args.bS, shuffle_train=True
    )
...
def train(
    ...
    for iB, t in enumerate(train_loader):
        ...
        # Calculate loss & step
        loss = Loss_sw_se(
            s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, g_sc, g_sa, g_wn, g_wc, g_wo, g_wvi
        )
        ...
        pr_sql_i = generate_sql_i(
            pr_sc, pr_sa, pr_wn, pr_wc_sorted, pr_wo, pr_wv_str, nlu
        )

        # Cacluate accuracy
        cnt_sc1_list, cnt_sa1_list, cnt_wn1_list, cnt_wc1_list, cnt_wo1_list, cnt_wvi1_list, cnt_wv1_list = get_cnt_sw_list(
            g_sc,
            g_sa,
            g_wn,
            g_wc,
...
def report_detail(
    ...
    cnt_tot, cnt, cnt_sc, cnt_sa, cnt_wn, cnt_wc, cnt_wo, cnt_wv, cnt_wvi, cnt_lx, cnt_x = (
        current_cnt
    )
    ...
    print(
        f"acc_lx = {cnt_lx/cnt:.3f}, acc_x = {cnt_x/cnt:.3f}\n",
...
def print_result(epoch, acc, dname):
    ave_loss, acc_sc, acc_sa, acc_wn, acc_wc, acc_wo, acc_wvi, acc_wv, acc_lx, acc_x = (
        acc
    )
...
if __name__ == "__main__":
    ...
    ## 2. Paths
    path_h = os.environ["HOME"]
    path_wikisql = os.path.join(path_h, "src", "wikisql_tok")
    BERT_PT_PATH = path_wikisql

    path_save_for_evaluation = "./"

    ## 3. Load data
    train_data, train_table, dev_data, dev_table, train_loader, dev_loader = get_data(
        path_wikisql, args
    )
